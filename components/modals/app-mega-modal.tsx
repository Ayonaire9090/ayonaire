...
interface AppMegaModalProps {
  ...
  headerContent?: React.ReactNode; // For the tab switchers
  footerContent?: React.ReactNode; // For sticky footer
}
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        ...
        className="max-w-[100vw] w-screen h-dvh max-h-dvh m-0 p-0 bg-[#F6F6F6] flex flex-col gap-0 border-none rounded-none shadow-none duration-200"
      >
        ...
        <div className="w-full flex flex-col px-4 md:px-6 py-4 border-b border-gray-200 shrink-0">
          <div className="flex justify-between max-w-3xl mx-auto w-full relative">
          ...
          {headerContent && (
            <div className="max-w-3xl mx-auto w-full mt-4 flex">
              {headerContent}
            </div>
          )}
        </div>
        ...
        {footerContent && (
          <div className="w-full bg-[#F6F6F6] border-t border-gray-200 p-4 px-4 md:px-6 fixed bottom-0 md:relative">
            <div className="max-w-3xl mx-auto w-full">{footerContent}</div>
          </div>
        )}
      </DialogContent>
    </Dialog>
