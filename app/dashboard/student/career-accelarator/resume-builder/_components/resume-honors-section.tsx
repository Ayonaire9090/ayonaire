...
const newEntry = (title = ""): HonorEntry => ({
...
type TextField = Exclude<keyof HonorEntry, "id" | "collapsed" | "description">;

const fields: { name: TextField; label: string; placeholder: string }[] = [
  { name: "title", label: "Award Title", placeholder: "Best Innovation Award" },
  { name: "issuingOrganization", label: "Issuing Organization", placeholder: "Tech Excellence Foundation" },
  { name: "dateReceived", label: "Date Received", placeholder: "June 2023" },
];

export function ResumeHonorsSection({
  entries,
  onChange,
  instruction,
  onInstructionChange,
  ...
  entries: HonorEntry[];
  onChange: (entries: HonorEntry[]) => void;
  instruction: string;
  ...
  const updateField = (id: string, name: keyof HonorEntry, value: string) =>
  ...
  const handleEnhance = () => {
    // No backend endpoint yet accepts a structured award entry for AI
    // enhancement — the existing career endpoints only generate free text.
    toast.error("AI enhancement for Honors & Awards isn't available yet — no backend endpoint accepts structured award data.");
  };

  return (
    <div className="flex flex-col gap-4">
      <h2 className="text-lg font-semibold text-gray-900">Honors &amp; Awards</h2>
      ...
      {entries.length === 0 ? (
        <>
          ...
          <p className="text-sm text-gray-400">
            No Honors &amp; Awards added yet. Click the button above to add one.
          </p>
        </>
      ) : (
      ...
      {entries.map((entry, index) => (
        <div key={entry.id} className="rounded-2xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between bg-[#FDEEEC] px-4 py-3">
            <p className="font-semibold text-gray-900">Award {index + 1}</p>
...
export { newEntry as newHonorEntry };
