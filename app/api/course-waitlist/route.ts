...
import { CourseWaitlistSchema } from "@/schemas/courses";
...
export async function POST(req: Request) {
  try {
    ...
    const validationResult = CourseWaitlistSchema.safeParse(body);
    ...
    const { fullName, email, course } = validationResult.data;
    ...
    if (existingResponse.ok) {
      ...
      const existingEntry = existingData.data?.find(
        (entry: { email: string }) =>
