    ...
    <section className="bg-gradient-to-b from-[#fefefe] to-[rgba(248,100,50,0.08)] px-5 py-16 sm:px-8 lg:px-12 lg:py-[120px] xl:px-16">
      <div className="mx-auto flex w-full max-w-[1240px] flex-col gap-14 lg:gap-20">
        <div className="flex flex-col items-center gap-8 text-center lg:gap-10">
          ...
          <p className={`${exo.className} w-full text-left text-[18px] font-medium leading-[1.65] tracking-[0.3px] text-[#263238] lg:text-[28px] lg:leading-[1.5]`}>
...
function CapabilityBlock({ title, columns }: { title: string; columns: string[][] }) {
  return (
    <div className="flex flex-col gap-8 lg:gap-12">
      <h3 className={`${exo.className} text-[26px] font-semibold uppercase leading-[1.25] tracking-[-0.4px] text-black sm:text-[32px] lg:text-[48px]`}>
        {title}
      </h3>
  ...
  return (
    <article className="rounded-[16px] bg-white px-6 py-8 shadow-[0_1px_10px_rgba(0,0,0,0.16)] sm:px-10 lg:px-[60px] lg:py-10">
      <ul className="grid gap-5 lg:gap-6">
        {items.map((item) => (
          <li key={item} className={`${exo.className} grid grid-cols-[30px_1fr] items-start gap-4 text-[17px] font-bold leading-[1.45] tracking-[0.2px] text-[#263238] sm:text-[20px] lg:text-[28px] lg:leading-[1.55]`}>
            <BulletPair />
            <span>{item}</span>
          </li>
        ))}
