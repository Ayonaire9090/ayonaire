      ...
      <div className="mx-auto max-w-[1240px] rounded-[40px] bg-[#fdd8cc] px-6 py-12 sm:px-10 lg:px-20 lg:py-20">
        <div className="flex flex-col items-center gap-10 lg:gap-11">
          <div className="w-full space-y-8">
            <p className={`${exo.className} text-[20px] font-bold uppercase leading-[1.6] tracking-[0.08em] text-[#5a4136] underline underline-offset-4 lg:text-[28px]`}>
              So, what does the week look like?
            </p>
            <p className={`${exo.className} text-[22px] font-bold leading-[1.65] tracking-[0.5px] text-[#181c23] sm:text-[26px] lg:text-[32px] lg:leading-[1.78]`}>
            ...
            {schedule.map((item) => (
              <div key={item.number} className="grid grid-cols-[72px_1fr] items-center gap-5 sm:grid-cols-[100px_1fr]">
                ...
                <div className={`${exo.className} space-y-3 text-[#181c23]`}>
                  <p className="text-[24px] font-medium uppercase tracking-[0.4px] lg:text-[32px]">{item.day}</p>
                  <p className="text-[22px] font-medium tracking-[0.4px] lg:text-[28px]">{item.time}</p>
                </div>
              </div>
            ))}
          </div>

          <p className={`${exo.className} max-w-[1100px] text-center text-[22px] font-bold leading-[1.65] tracking-[0.5px] text-[#181c23] underline underline-offset-4 sm:text-[26px] lg:text-[32px] lg:leading-[1.78]`}>
