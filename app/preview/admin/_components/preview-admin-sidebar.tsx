...
const PREVIEW_ROOT = "/preview/admin";

// Same nav data as the real admin sidebar (components/dashboard/app-sidebar.tsx),
// just with URLs remapped from /dashboard/admin/* to /preview/admin/* so
// clicking a nav item stays inside the auth-free preview instead of
// bouncing into the real, guarded dashboard.
const previewNavItems = dashboardData.adminNavMain.map((item) => ({
  ...item,
  url: item.url.replace("/dashboard/admin", PREVIEW_ROOT),
}));
...
export function PreviewAdminSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
